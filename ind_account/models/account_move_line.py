...
class AccountMoveLine(models.Model):
    ...
    def _compute_tax_amount(self):
        for line in self:
            taxes_res = line.tax_ids.compute_all(line.price_unit, line.move_id.currency_id, line.quantity, product=line.product_id, partner=line.move_id.partner_id)
            line.tax_amount = sum(tax['amount'] for tax in taxes_res['taxes']) if taxes_res else 0.0
    
    def _generate_price_difference_vals(self, layers):
        logger.warning('----price_diffe----')
        '''
        ...
        for layer in layers:
            ...
            if self.is_refund:
                ...
                logger.warning('---cantidades----')
                logger.warning(invoicing_layer_qty)
                logger.warning(out_qty_to_invoice)
                logger.warning(qty_to_correct)
            ...
            logger.warning('---halla diferencia----')
            logger.warning(aml_price_unit)
            logger.warning(layer_price_unit)
            logger.warning(unit_valuation_difference)
            ...
            logger.warning('---diferencia precio----')
            logger.warning(po_pu_curr)
            logger.warning(aml_gross_price_unit)
            logger.warning(price_difference_curr)
