...
class Main(http.Controller):
    ...
    @http.route('/reports/type_format/<string:object>/<string:function_name>/<string:file_format>/<int:ids>', type='http', auth='user')
    def report_function_formats(self, object, function_name, file_format, ids, **kw):
        Model = request.env[object].browse(ids)
        content = Model.document_print(function_name)
        filename = Model.file_name(file_format, function_name)
        return request.make_response(content, [('Content-Type', self._get_content_type_format_print(file_format)),
                                               ('Content-Disposition', content_disposition(filename))])
