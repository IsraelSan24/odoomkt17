...
class AccountMove(models.Model): 
    ...
    def _default_l10n_pe_edi_detraction_payment_type(self):
        company_id = self.env.company
        l10n_pe_edi_detraction_payment_type_id = (
            company_id.l10n_pe_edi_detraction_payment_type_id and
            company_id.l10n_pe_edi_detraction_payment_type_id.id or False
        )
        return l10n_pe_edi_detraction_payment_type_id
    ...
    l10n_pe_edi_retention_type_id = fields.Many2one('l10n_pe_edi.catalog.23', string='Retention Type', copy=False)
    l10n_pe_edi_detraction_type_id = fields.Many2one('l10n_pe_edi.catalog.54', string='Detraction Type', copy=False)
    l10n_pe_edi_detraction_payment_type_id = fields.Many2one('l10n_pe_edi.catalog.59', string='Detraction Payment Type', copy=False,
        default=_default_l10n_pe_edi_detraction_payment_type)
    ...
    l10n_pe_edi_total_retention = fields.Monetary(string='Total Retention', store=True, compute='_compute_retention', tracking=True)
    l10n_pe_edi_amount_total_retention = fields.Monetary(string='Amount Total with Retention', store=True, compute='_compute_retention', tracking=True)
    l10n_pe_edi_total_detraction = fields.Monetary(string='Total Detraction', store=True, compute='_compute_detraction', tracking=True)
    l10n_pe_edi_total_detraction_signed = fields.Monetary(string='Total Detraction Signed', store=True, compute='_compute_detraction', currency_field='company_currency_id', tracking=True)
    l10n_pe_edi_amount_total_detraction = fields.Monetary(string='Amount Total with Detraction', store=True, compute='_compute_detraction', tracking=True)
    ...
    l10n_pe_edi_sunat_canceled = fields.Boolean('Canceled by SUNAT', related='l10n_pe_edi_request_id.sunat_canceled', store=True, copy=False)
    l10n_pe_edi_sunat_canceled_progress = fields.Boolean('Cancellation is in process', store=True, copy=False)
    progress_message = fields.Char('Progress Message', copy=False, readonly=True)
    ...
    @api.depends('l10n_pe_edi_retention_type_id', 'l10n_pe_edi_amount_base', 'amount_total')
    def _compute_retention(self):
        self.l10n_pe_edi_total_retention = 0.0
        for move in self:
    ...
    @api.depends('l10n_pe_edi_detraction_type_id', 'l10n_pe_edi_detraction_type_id.rate', 'amount_total', 'amount_total_signed')
    def _compute_detraction(self):
        self.l10n_pe_edi_total_detraction = 0.0
        self.l10n_pe_edi_total_detraction_signed = 0.0
        for move in self:
            if move.l10n_pe_edi_detraction_type_id and move.l10n_pe_edi_detraction_type_id.rate:
                move.l10n_pe_edi_total_detraction = round(move.amount_total * (move.l10n_pe_edi_detraction_type_id.rate / 100), 0)
                detraction_signed = move.l10n_pe_edi_total_detraction
                if move.currency_id != move.company_id.currency_id:
                    detraction_signed = move.currency_id._convert(detraction_signed, move.company_id.currency_id, move.company_id, move.date or fields.Date.context_today(move))
    ...
    def _get_detraction_type_lines(self):
        detraction = False
        detractions = self.invoice_line_ids.mapped('product_id.l10n_pe_edi_detraction_type_id')
        #Only for peruvian clients
        if self.partner_id.commercial_partner_id.country_id and self.partner_id.commercial_partner_id.country_id.code == "PE":
            #If the detraction amount is greater than S/. 700.00
            if detractions and self.amount_total_signed >= self.company_id.l10n_pe_edi_min_amount_detraction:
                detraction = detractions.sorted('rate', reverse=True)
                detraction = detraction[0]
        return detraction
    
    def _set_detraction_type(self):
        if not self.l10n_pe_edi_detraction_type_id:
            self.l10n_pe_edi_detraction_type_id = self._get_detraction_type_lines()
    ...
    def _compute_edi_amount(self):
        for move in self:
            ...
            # move.l10n_pe_edi_amount_base = sum([x[2] for x in move.tax_totals_json if x[0] not in ['INA','EXO','EXP']])
            ...
            move.l10n_pe_edi_amount_igv = sum([x['tax_amount'] for x in tax_lines_data if x.get('tax_amount', False) and x['tax'].tax_group_id.name == 'IGV'])
            move.l10n_pe_edi_amount_isc = sum([x['tax_amount'] for x in tax_lines_data if x.get('tax_amount', False) and x['tax'].tax_group_id.name == 'ISC'])
            move.l10n_pe_edi_amount_icbper = sum([x['tax_amount'] for x in tax_lines_data if x.get('tax_amount', False) and x['tax'].tax_group_id.name == 'ICBPER'])
            move.l10n_pe_edi_amount_others = sum([x['tax_amount'] for x in tax_lines_data if x.get('tax_amount', False) and x['tax'].tax_group_id.name == 'OTROS'])
    ...
    @api.onchange('l10n_pe_edi_is_sale_credit', 'l10n_pe_edi_retention_type_id', 'l10n_pe_edi_detraction_type_id')    
    def _get_dues_ids(self):
        if self.l10n_pe_edi_is_sale_credit:
            self.l10n_pe_edi_dues_ids = False
            number = 0
            dues = []
            lines = self.line_ids.filtered(lambda x: x.exclude_from_invoice_tab and x.date_maturity != False and x.debit > 0.0)
            detraction_line = self.l10n_pe_edi_total_detraction / len(lines)
            retention_line = self.l10n_pe_edi_total_retention / len(lines)
            for line in lines:
                number += 1
                amount = abs(line.amount_currency)
                if self.l10n_pe_edi_retention_type_id:
                    amount = amount - retention_line
                if self.l10n_pe_edi_detraction_type_id:
                    amount = amount - detraction_line
                vals = {
                    'move_id': self.id,
                    'dues_number': number,
    ...
    def action_post(self):
        ...
        for move in self:
            if move.l10n_pe_edi_retention_type_id:
                if move.l10n_latam_document_type_id.type_of != '1':
                    raise UserError(_('The retention can only be applied to invoices.'))
                if move.amount_total_signed <= move.company_id.l10n_pe_edi_min_amount_retention:
                    raise UserError(_('The retention can only be applied for amounts greater than 700'))
            
            if move.l10n_pe_edi_detraction_type_id:
                if move.l10n_latam_document_type_id.type_of != '1':
                    raise UserError(_('The detraction can only be applied to invoices.'))
            else:
                if self._get_detraction_type_lines():
                    raise UserError(_("Detraction is mandatory for amounts greater than S/. 700.00"))
                
            amount = move.l10n_pe_edi_dues_ids.mapped('amount')
            if move.l10n_pe_edi_is_sale_credit and move.l10n_pe_edi_retention_type_id:
                if sum(amount) != move.l10n_pe_edi_amount_total_retention:
                    raise UserError(_('The sum of the dues must be equal to the Net Amount pending payment.'))
            if move.l10n_pe_edi_is_sale_credit and move.l10n_pe_edi_detraction_type_id:
                if sum(amount) != move.l10n_pe_edi_amount_total_detraction:
                    raise UserError(_('The sum of the dues must be equal to the Net Amount pending payment.'))
    ...
    def _get_invoice_values_odoofact(self):
        ...
        values = {
            ...
            "medio_de_pago_detraccion": self.l10n_pe_edi_detraction_type_id and self.l10n_pe_edi_detraction_payment_type_id and
                                            int(self.l10n_pe_edi_detraction_payment_type_id.code_of) or '',
    ...
    def action_document_send(self):
        ...
        for move in self:
            ...
            if move.l10n_pe_edi_operation_type == '4' and (move.l10n_pe_edi_retention_type_id or move.l10n_pe_edi_detraction_type_id):
                raise UserError(_("Retention or Detraction cannot be applied on a invoice with advance, " +
                    "You must apply it afer submitting it to SUNAT so that it is displayed informatively only in the system."))
            if move.l10n_pe_edi_retention_type_id and move.l10n_pe_edi_detraction_type_id:
                raise UserError(_("Retention and Detraction cannot be applied at the same time."))
            
            amount = self.l10n_pe_edi_dues_ids.mapped('amount')
            if move.l10n_pe_edi_is_sale_credit and move.l10n_pe_edi_retention_type_id:
                if sum(amount) != move.l10n_pe_edi_amount_total_retention:
                    raise UserError(_('The sum of the dues must be equal to the Net Amount pending payment.'))
            if move.l10n_pe_edi_is_sale_credit and move.l10n_pe_edi_detraction_type_id:
    ...
    def action_document_send_cancel(self):
        ...
        for move in self:
            ...
            if not move.l10n_pe_edi_sunat_accepted:
                raise UserError(_("It is not possible to cancel a voucher if it has not been accepted by SUNAT. Please check the status before."))
            ...
            move.progress_message = _("Voucher cancellation is in process.")
            move.l10n_pe_edi_sunat_canceled_progress = True
            ...
            elif move.state != 'cancel' and move.l10n_pe_edi_response:
    ...
    def action_document_check(self, cancel=False):
        ...
        for move in self:
            ...
            if move.l10n_pe_edi_request_id:
                move.l10n_pe_edi_request_id.action_api_connect(vals)
                print(move.l10n_pe_edi_request_id.sunat_canceled)
                if move.l10n_pe_edi_request_id.sunat_canceled == True:
                    move.button_draft()
                    move.button_cancel()
                    move.progress_message = ''
                    move.l10n_pe_edi_sunat_canceled_progress = False
...
class AccountMoveLine(models.Model):
    ...
    l10n_pe_edi_price_unit_excluded = fields.Float(string='Price unit excluded', store=True, readonly=True, help="Price unit without taxes")
    l10n_pe_edi_price_unit_included = fields.Float(string='Price unit IGV included', store=True, readonly=True, help="Price unit with IGV included")
    ...
    l10n_pe_edi_amount_free = fields.Float(string='Amount free', store=True, readonly=True, help='amount calculated if the line id for free product')
