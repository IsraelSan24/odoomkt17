...
def _create_journal_debit(env):
    ...
    for journal in env['account.journal'].search([('type','=','sale'),('company_id','in',company_ids.ids)]):
        journal.write({
                    ...
                    'l10n_latam_document_type_id': env.ref('l10n_pe.document_type01').id,
    ...
    company_with_debit_note_journal = env['account.journal'].search([('l10n_latam_document_type_id','=',env.ref('l10n_pe.document_type08').id)]).mapped('company_id')
    ...
    for company in company_without_debit_note_journal:
        env['account.journal'].create({
                        ...
                        'type': 'sale',
                        'l10n_pe_edi_is_einvoice': True, 
                        'l10n_latam_document_type_id': env.ref('l10n_pe.document_type08').id,
...
def _set_min_amount_detraction_retention(env):
    company_ids = env['res.company'].search([]).filtered(lambda r: r.country_id.code == 'PE')
    for company_id in company_ids:
        company_id.write({
            "l10n_pe_edi_min_amount_retention": 700.00,
            "l10n_pe_edi_min_amount_detraction": 700.00,
        })
...
def _l10n_pe_edi_odoofact_init(cr, registry):
    ...
    _set_min_amount_detraction_retention(env)
